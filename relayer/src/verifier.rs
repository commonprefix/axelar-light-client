...
use eyre::Result;
use crate::{types::{LightClientStateResult, UpdateExecuteMsg, UpdateMsg}, utils::calc_sync_period};
...
impl Verifier {
    ...
    pub async fn update(&self, update: Update) -> Result<()> {
        ...
        let message = UpdateExecuteMsg {
            LightClientUpdate: UpdateMsg {
                update
            }
        };

        let args = [
            "tx", "wasm", "execute", self.address.as_str(),
            &serde_json::to_string(&message)?,
            "--from", "pkakelas",
            "--node", self.rpc.as_str(),
            "--gas-prices", "0.0001uwasm",
            "--gas", "100000000",
            "-y"
        ];
